...
function main(comboID,R_E,R_I)
    ...
    if(exist(fullfile(folder,'presynaptic_network','covariance.mat')))
        ...
        calculate_R(comboID,R_E,R_I,folder);
        return;
    end


    if(exist(fullfile(network.preNetwork,'chol_var.mat')))
        ...
        % #SBATCH --mem-per-cpu=16G
        load(fullfile(network.preNetwork,'covariance.mat'),'L0');
        Lc = chol(nearcorr(L0),'lower');
        save(fullfile(network.preNetwork,'chol_var.mat'),'Lc','-v7.3')
        return;
    end
...
function calculate_R(comboID,R_E,R_I,folder)
